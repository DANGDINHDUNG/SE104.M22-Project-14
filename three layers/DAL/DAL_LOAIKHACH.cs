...
namespace DAL
{
    public class DAL_LOAIKHACH:KetNoi
    {
        ...
        {
            connection.Open();
            string sql = string.Format("INSERT INTO LOAIKHACH(MALK,TENLOAIKHACH) VALUES ('{0}', '{1}')",lk.MALK,lk.TENLOAIKHACH);
        ...
        {
            connection.Open();
            string sql = string.Format("UPDATE LOAIKHACH SET  TENLOAIKHACH='{0}'",lk.TENLOAIKHACH);
            SqlCommand cmd = new SqlCommand(sql, connection);
            if (cmd.ExecuteNonQuery() > 0)
                return true;
            else return false;
